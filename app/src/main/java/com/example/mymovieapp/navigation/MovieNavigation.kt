...
fun MovieNavigation(){
    ...
    NavHost(navController = navController, startDestination = "HomeScreen" /*hier startet die App*/){
        ...
        composable(MovieScreens.HomeScreen.name) { HomeScreen(navController = navController)} // --> verlinkung von String (enum.name) mit einzigartigen Composable
        composable(MovieScreens.FavoritesScreen.name) { FavoriteScreen(navController = navController)}

        //argumente werden wie in url verarbeitet, url: www.domain.com/detailscreen/movie=12
        composable(MovieScreens.DetailScreen.name +"/{movie}", //konkret wird eine ID übergeben
